...
<body>
	...
	<div class="container">
		<img src="../resources/images/logo.png" alt="Course-banner" width="40%" height="250px"/>
		<h3 class="h3 text-center my-5">${course.courseName}</h3>
		
		<h4 class="h4">About the Course</h4>
		<p style="font-size: 20px;">${course.about}</p>
		...
		<div style="height: 600px; overflow-x: hidden; overflow-y: scroll;  padding: 20px;">
			  ...
			  <tbody>
					<c:forEach begin="0" end="${noOfDays}" step="1">
				   		<c:forEach var="batch" items="${batches}" varStatus="batchStatus"> 
								  ...
								  <td>${batch.timings}</td>
								  <c:if test="${batchStatus.index eq 0}">
							        <td rowspan="${countOfBatches}" style="vertical-align: middle;">${course.fees}</td>
							      </c:if>
